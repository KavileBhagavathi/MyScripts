...
int main(int argc, char** argv)
{
  char* cores = argv[3];
  char filename[32];
  snprintf(filename, sizeof(filename), "raytrace100_core%s.txt", cores);
  FILE *fptr;
  fptr = fopen(filename,"w");
  fprintf(fptr,"Cores,Total_WallTime,Performance\n");
  ...
  float performance = (size*size)/(we-ws);
  fprintf(fptr,"%s,",cores);
  fprintf(fptr,"%.4lf,",we-ws);
  fprintf(fptr,"%f\n",performance);
  fclose(fptr);
  fclose(fd);
  free(tile);
